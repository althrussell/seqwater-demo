...
class Settings(BaseSettings):
    ...
    databricks_workspace_id: str | None = Field(
        default=None, alias="DATABRICKS_WORKSPACE_ID"
    )
    ...
    def genie_embed_url_resolved(self) -> str | None:
        ...
        1. The explicit ``DATABRICKS_GENIE_EMBED_URL`` (recommended — paste the
           ``src`` from the iframe generated by *Share → Embed space* inside
           the Genie UI).
        2. ``{host}/embed/genie/rooms/{space_id}?o={workspace_id}`` constructed
           from the workspace host, space ID, and (optional but strongly
           recommended) workspace organisation id. This matches the shape
           produced by the Genie *Embed space* dialog.
        ...
        if "://" not in host:
            host = f"https://{host}"
        url = f"{host}/embed/genie/rooms/{space}"
        workspace_id = (self.databricks_workspace_id or "").strip()
        if workspace_id:
