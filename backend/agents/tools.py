...
def get_water_security_summary() -> dict[str, Any]:
    ...
    summary_text = (
        "SEQ-wide storage at {storage:.1f}%. "
        "Mean 72h rainfall forecast across catchments: {rain:.0f} mm. "
        "Today's demand: {dem:.0f} ML/day. "
...
def get_top_asset_risks(limit: int = 5) -> dict[str, Any]:
    ...
    if not risks:
        return {"summary": "No asset risk data available.", "data": []}
    lines = [
        f"{r['asset_name']} ({r['risk_band']}, score {r['risk_score']:.2f}) — {r['recommended_action']}"
        for r in risks
    ]
    summary = "Top asset risks:\n- " + "\n- ".join(lines)
...
def get_water_quality_alerts() -> dict[str, Any]:
    ...
    summary = (
        ...
        f"Latest turbidity events: {len(q['turbidity_events'])}."
    )
...
def run_flood_readiness_scenario(scenario_id: str = "FS-001") -> dict[str, Any]:
    ...
    if not s:
        return {"summary": "No scenario detail available.", "data": {}}
    summary = (
        f"Scenario {s.get('scenario_name')}: 72h forecast {s.get('rainfall_forecast_mm_72h')} mm; "
...
def get_capital_priorities(limit: int = 5) -> dict[str, Any]:
    ...
    if not projects:
        return {"summary": "No capital projects available.", "data": []}
    lines = [
    ...
    summary = "Top capital options by risk reduction:\n- " + "\n- ".join(lines)
...
def get_current_dam_levels(dam_name: str | None = None) -> dict[str, Any]:
 ...
 time series. When ``dam_name`` is provided, the response narrows
